...
class TrainConfig:
    ...
    # ── Data ─────────────────────────────────────────────────
    stable_dir:        str   = "./data/data/stable"
    ...
    eval_tokens:       int   = 4_096_000    # ~2k steps worth of eval data
...
def evaluate(
    ...
    model.eval()
    ds     = ShardedTokenDataset(shard_dir, seq_len=seq_len, shuffle_shards=False)
...
def train(cfg: TrainConfig):
    ...
    if cfg.compile_model and "cuda" in device:
        if is_main:
            print("[Compile] Running torch.compile (first step will be slow)...")
...
def parse_args() -> Tuple[TrainConfig, bool]:
    ...
    # ── Data ─────────────────────────────────────────────────
    parser.add_argument("--stable_dir",    default="./data/data/stable")
