...
=====================
Defines BOTH models for the mechanistic interpretability comparison study.

  Model A — BaselineTransformer
    Full decoder-only Transformer using Multi-head Latent Attention (MLA).
    MLA replaces standard MHA with low-rank KV compression — cutting the
    KV cache by ~81% at inference while matching or exceeding MHA quality.
    The compressed latent c_KV is a natural interpretability probe: it is
    the bottleneck through which ALL key/value information must pass, making
    it an ideal target for activation patching in Phase 3.

  Model B — MambaHybrid
    Mamba-2 SSM layers (43%) + MLA-Attention layers (7%) + SwiGLU MLP (50%).
    The attention layers use the SAME MLA implementation as Model A — so any
    circuit found in Model A can be directly compared against Model B.

Shared components:
  RMSNorm       — no mean subtraction, no bias, faster than LayerNorm
  RoPE          — applied only to the rope sub-dimension of Q/K heads
                  (LLaMA-3 style base=500_000 for long-context generalisation)
  MLA           — Multi-head Latent Attention (DeepSeek-V2 style)
                  • W_DKV projects x -> c_KV  (kv_lora_rank dims)  [CACHED]
                  • W_UK / W_UV decompress c_KV -> K, V
                  • W_DQ  projects x -> c_Q   (q_lora_rank dims)
                  • W_UQ / W_QR decompress c_Q -> q_nope, q_rope
                  • RoPE applied to q_rope and k_rope only
                  • QK-RMSNorm on full head vectors after cat(nope, rope)
                  • GQA repeat: n_kv_heads -> n_heads
                  • Value residual: x -> W_VR -> added to attn output
  SwiGLU MLP    — gated FFN used in both models
  muP init      — fan-in scaled init for clean LR transfer from small runs
"""
